...
import { Loader2, FileText, MapPin, Building, Ruler, Shield, Leaf, DollarSign, Check, AlertCircle } from "lucide-react"
import "@/styles/report.css"
import { Button } from "@/components/ui/button"
import { Input } from "@/components/ui/input"
import { useRequestForLocation } from "@/hooks/useRequestForLocation"

export function GeneralPropertyFallbackTab() {
  const [email, setEmail] = useState("")
  const { sendLocationRequest, loading, error, isSubmitted } = useRequestForLocation()
  
  const sampleSections = [
    {
      title: "Property Identification & Legal Framework",
      icon: <FileText className="h-5 w-5 text-primary" />,
      datapoints: ["Geospatial Information", "Legal Description", "Zoning Classification", "Tax Status", "tax Delinquincy"]
    },
    {
      title: "Physical Site Characteristics",
      icon: <MapPin className="h-5 w-5 text-primary" />,
      datapoints: ["Lot Configuration", "Easements & Encumbrances", "Structural Inventory"]
    },
    {
      title: "Zoning & Entitlements",
      icon: <Ruler className="h-5 w-5 text-primary" />,
      datapoints: ["Entitlement Status", "Current Approvals", "Required Permits"]
    },
    {
      title: "Construction & Systems Profile",
      icon: <Building className="h-5 w-5 text-primary" />,
      datapoints: ["Structural Components", "Utilities"]
    },
    {
        title: "Environmental & Geotechnical",
        icon: <Leaf className="h-5 w-5 text-primary" />,
        datapoints: ["Environmental Assessment", "Phase I ESA Findings", "Soil Contaminations", "Flood Risk Assessment", "FEMA Designation", "Mitigation Requirements"]
    },
    {
        title: "Development Economics",
        icon: <DollarSign className="h-5 w-5 text-primary" />,
        datapoints: ["Current Use Analysis", "Tax History & Projections", "Reuse Potential"]
    },
    {
        title: "Insurance & Risk Assessment",
        icon: <Shield className="h-5 w-5 text-primary" />,
        datapoints: ["Insurance Profile", "Risk Assessment Matrix"]
    }
  ]
